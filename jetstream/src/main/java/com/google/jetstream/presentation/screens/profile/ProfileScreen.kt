...
fun ProfileScreen(
    ...
    Row(
        ...
        var isCarouselEnabled by rememberSaveable { 
            mutableStateOf(preferences.carouselEnabled) 
        }
        var isImmersiveListEnabled by rememberSaveable { 
            mutableStateOf(preferences.immersiveListEnabled) 
        }
        var isDynamicBackgroundColorEnabled by rememberSaveable { 
            mutableStateOf(preferences.dynamicBackgroundColorEnabled) 
        }
        var isBlackBackgroundEnabled by rememberSaveable { 
            mutableStateOf(preferences.blackBackgroundEnabled) 
        }
        
        // Update preference when carousel setting changes
        LaunchedEffect(isCarouselEnabled) {
            preferences.carouselEnabled = isCarouselEnabled
        }
        
        // Update preference when immersive list setting changes
        LaunchedEffect(isImmersiveListEnabled) {
            preferences.immersiveListEnabled = isImmersiveListEnabled
        }
        
        // Update preference when dynamic background color setting changes
        LaunchedEffect(isDynamicBackgroundColorEnabled) {
            preferences.dynamicBackgroundColorEnabled = isDynamicBackgroundColorEnabled
        }
        
        // Update preference when black background setting changes
        LaunchedEffect(isBlackBackgroundEnabled) {
            preferences.blackBackgroundEnabled = isBlackBackgroundEnabled
        }
        ...
        NavHost(
            ...
            builder = {
                composable(ProfileScreens.Accounts()) {
                    AccountsSection(
                        onLogout = {
                            // This will be handled by App composable observing login state
                        }
                    )
                }
                composable(ProfileScreens.About()) {
                    AboutSection(
                        ...
                        onCarouselEnabledChange = { isCarouselEnabled = it },
                        isImmersiveListEnabled = isImmersiveListEnabled,
                        onImmersiveListEnabledChange = { isImmersiveListEnabled = it },
                        isDynamicBackgroundColorEnabled = isDynamicBackgroundColorEnabled,
                        onDynamicBackgroundColorEnabledChange = { isDynamicBackgroundColorEnabled = it },
                        isBlackBackgroundEnabled = isBlackBackgroundEnabled,
                        onBlackBackgroundEnabledChange = { isBlackBackgroundEnabled = it }
                    )
                }
